...
train_loader = get_loader(
    image_root, depth_root, gt_root, batchsize=opt.batchsize, trainsize=opt.trainsize
)
...
def train(train_loader, model, optimizer, epoch, save_path):
    ...
    try:
        for i, (images, depths, gts) in enumerate(train_loader, start=1):
            ...
            if i % 50 == 0:
                print(
                    "{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Loss: {:.6f}".format(
                        datetime.now(),
                        epoch,
                        opt.epoch,
                        i,
                        total_step,
                        loss_all.data / epoch_step,
                    )
                )
        loss_all /= epoch_step
        print(
            "Epoch [{:03d}/{:03d}]:Loss_AVG={:.6f}".format(epoch, opt.epoch, loss_all)
        )
        logging.info(
        ...
        if epoch > 60 and (epoch % 5 == 0 or epoch == opt.epoch):
            torch.save(
                model.state_dict(), save_path + "CIRNet_epoch_{}.pth".format(epoch)
            )
    except KeyboardInterrupt:
        print("Keyboard Interrupt: save model and exit.")
        torch.save(
            model.state_dict(),
            save_path + "CIRNet_epoch_{}_checkpoint.pth".format(epoch + 1),
        )
