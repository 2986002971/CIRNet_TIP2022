...
class CIRNet_StarNet(nn.Module):
    ...
    Modified to use StarNet S4 as backbone
    """
    ...
    def __init__(self, norm_layer=nn.BatchNorm2d):
        ...
        # StarNet的通道数
        star_channels = [32, 32, 64, 128, 256]
        # 目标通道数保持不变
        channels = [64, 128, 256, 512, 512]

        # layer 1
        self.re1_r = BaseConv2d(star_channels[0], channels[0], kernel_size=1)
        self.re1_d = BaseConv2d(star_channels[0], channels[0], kernel_size=1)

        # layer 2
        self.re2_r = BaseConv2d(star_channels[1], channels[1], kernel_size=1)
        self.re2_d = BaseConv2d(star_channels[1], channels[1], kernel_size=1)

        # layer 3
        self.re3_r = BaseConv2d(star_channels[2], channels[2], kernel_size=1)
        self.re3_d = BaseConv2d(star_channels[2], channels[2], kernel_size=1)

        self.conv1 = BaseConv2d(2 * channels[2], channels[2], kernel_size=1)
        self.sa1 = SpatialAttention(kernel_size=7)

        # layer 4
        self.re4_r = BaseConv2d(star_channels[3], channels[3], kernel_size=1)
        self.re4_d = BaseConv2d(star_channels[3], channels[3], kernel_size=1)

        self.conv2 = BaseConv2d(2 * channels[3], channels[3], kernel_size=1)
        self.sa2 = SpatialAttention(kernel_size=7)

        # layer 5
        self.re5_r = BaseConv2d(star_channels[4], channels[4], kernel_size=1)
        self.re5_d = BaseConv2d(star_channels[4], channels[4], kernel_size=1)
    ...
    def forward(self, rgb, depth):
        ...
        conv4_rgbd = F.interpolate(
            conv4_rgbd, scale_factor=1 / 2, mode="bilinear", align_corners=True
        )
        ...
        rgb_map, depth_map, rgbd_map = self.decoder(
            decoder_rgb_list, decoder_depth_list, rgbd_cmWR
        )
